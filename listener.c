...
static char *host_v6 = "2401:2000:6660::122";
static char *host_v4 = "192.168.56.1";
...
void listen_for_icmp(u_char *args, const struct pcap_pkthdr *header, const u_char *packet) {
  ...
  if (ipv4) {
    ipv4_hdr = (struct ip_hdr *)(packet + 14); 
    if (debug) {
      print_ipv4_header((const u_char *)ipv4_hdr);
      print_icmp_header((const u_char *)ipv4_hdr+IP_HDR_LEN);
    }
    // TODO IPv4
    return;
  }
  ...
  if (icmp6_hdr->icmp6_type == ICMP6_TIME_EXCEEDED || icmp6_hdr->icmp6_type == ICMP6_DST_UNREACH) {
    ...
    printf("%s,%s,%d,%d.%d",target_addr_str, icmp_src_addr_str, icmp6_hdr->icmp6_type, now.tv_sec, now.tv_usec);
...
int main (int argc, char **argv)
{
  ...
  while (((ch = getopt(argc,argv, "di:k:l:"))) != -1) {
    switch(ch) {
