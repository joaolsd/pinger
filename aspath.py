...
iter=0
path_valid = defaultdict(init_list)
path_invalid = defaultdict(init_list)
for host in sys.argv[1:]:
  ...
  with (open(valid_input_file)) as f:
    ...
    for line in Lines:
      ...
      path_valid[target_ip][0] = target_asn # Store ASN of target IP at hop 0
      ...
      hop_asn=int(hop_asn)
      try:
        path_valid[target_ip][hop] = hop_asn
      except IndexError:
  ...
  with (open(invalid_input_file)) as f:
    ...
    for line in Lines:
      ...
      addr,target_asn = addr_asn.strip().split(',')
      target_asn=int(target_asn)
      path_invalid[target_ip][0] = target_asn # Store ASN of target IP at hop 0
      ...
      hop_asn=int(hop_asn)
      try:
        path_invalid[target_ip][hop] = hop_asn
      except IndexError:
        print("TTL/hop too big:", hop)
        # I guess some packets get mangled, badly, by middle boxes.
        continue
...
with open("as_paths.txt", "w") as f:
  ...
  for target_ip in path_valid:
    target_ip_str = str(target_ip)
    print("{:<15}".format(target_ip_str)+":"+",".join("{:6d}".format(a) for a in path_valid[target_ip]), file=f)
    print(' '*15,','.join("{:6d}".format(a) for a in path_invalid[target_ip]), file=f)
